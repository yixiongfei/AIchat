...
async function waitForTipsEl(timeoutMs = 3000) {
...
// ✅ 普通显示（一次性文本）
export async function showWaifuMessage(text, timeout = 8000, priority = 10, override = true) {
    if (!text)
        return;
    ...
    if ((override && currentPriority > priority) || (!override && currentPriority >= priority))
        return;
...
export async function showWaifuStreamUpdate(fullText, opts) {
    ...
    const flush = async () => {
        ...
        await showWaifuMessage(streamState.pendingText, opts?.timeout ?? 10000, // 流式时长一点，避免中途气泡消失
        opts?.priority ?? 10, opts?.override ?? true);
    };
...
export function clearWaifuTimers() {
    if (hideTimer != null)
        window.clearTimeout(hideTimer);
    hideTimer = null;
    if (streamState.timer != null)
        window.clearTimeout(streamState.timer);
    streamState.timer = null;
}
