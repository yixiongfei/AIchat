...
export function isLongCode(lines, chars, thresholds = DEFAULT_THRESHOLDS) {
    return (lines >= thresholds.CODE_LINE_THRESHOLD ||
 ...
 */
export function segmentMessageByCodeBlocks(text, thresholds = DEFAULT_THRESHOLDS) {
    const re = /```(\w+)?\n([\s\S]*?)```/g;
    let lastIndex = 0;
    let codeIndex = 0;
    let mdBuffer = "";
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // 追加 code 前的文本
        if (start > lastIndex)
            mdBuffer += text.slice(lastIndex, start);
        codeIndex += 1;
        ...
        if (isLongCode(lines, chars, thresholds)) {
            ...
            if (mdBuffer.length > 0)
                segs.push({ type: "markdown", content: mdBuffer });
            mdBuffer = "";
            // 输出长代码段
            segs.push({
                type: "longCode",
    ...
    if (lastIndex < text.length)
        mdBuffer += text.slice(lastIndex);
    if (mdBuffer.length > 0)
        segs.push({ type: "markdown", content: mdBuffer });
    return mergeAdjacentMarkdown(segs);
}
function mergeAdjacentMarkdown(segs) {
    const out = [];
    for (const s of segs) {
...
export function stripAllFencedCodes(text) {
    return text.replace(/```[\s\S]*?```/g, "");
}
export function previewText(text, maxChars = 240) {
