...
// ========================================
function sanitizeForTTS(markdown) {
    if (!markdown)
        return "";
    let s = markdown;
    // ---------- 1) 去代码 ----------
...
// ========================================
function trimOverlap(prevTail, nextChunk, maxCheck = 160) {
    if (!prevTail || !nextChunk)
        return nextChunk;
...
function createMarkdownCodeFilter() {
    const st = { mode: "text", tickRun: 0, fenceHeader: false };
    const reset = () => {
        st.mode = "text";
        st.tickRun = 0;
...
function shouldSendTTSSegment(buffer, config) {
    const { minLength = 15, sentenceLength = 25, maxLength = 500, pauseLength = 25, } = config || {};
...
export default function useTTS(roleConfig) {
    ...
    const buildCacheKey = (message, cfg) => {
    ...
    const markSeqFailedAndContinue = useCallback((seq) => {
    ...
    const stop = useCallback(() => {
        ...
        cleanupAudio(audioRef.current);
        audioRef.current = null;
        for (const [, a] of audioQueueRef.current.entries())
            cleanupAudio(a);
    ...
    const getOrCreateAudioUrl = useCallback(async (message, cfg, signal) => {
        pruneCache();
    ...
    const enqueue = useCallback(async (message, opts = {}) => {
        if (!message?.trim())
            return;
    ...
    // ========================================
    const appendStream = useCallback((chunk, opts) => {
        const { minLength = 15, sentenceLength = 20, maxLength = 150, pauseLength = 50, debounceMs = 500, filterCode = true, debug = false, } = opts || {};
        // 1) ✅ 过滤代码（支持跨 chunk）
        ...
        if (decision.shouldSend) {
            ...
            if (clean) {
                if (debug) {
                    console.log(`[TTS Stream] ${decision.reason}: "${clean.slice(0, 60)}${clean.length > 60 ? "..." : ""}"`);
                }
            ...
            streamBufferRef.current.timer = window.setTimeout(() => {
                ...
                if (myToken !== timerTokenRef.current)
                    return;
                const [toSend, remaining] = extractSegment(streamBufferRef.current.buffer);
                streamBufferRef.current.buffer = remaining;
                streamBufferRef.current.timer = null;
                ...
                if (clean) {
                    if (debug) {
                        console.log(`[TTS Stream] timeout_flush: "${clean.slice(0, 60)}${clean.length > 60 ? "..." : ""}"`);
                    }
                    // ✅ 重要：更新 lastTailRef 为已发送的内容，防止重复
                    lastTailRef.current = toSend.slice(-240);
