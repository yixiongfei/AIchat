...
/**
 * 全局 loading 状态管理
 * 用于跟踪每个 agent 的后台请求状态
 * 当切换 agent 时，可以知道原 agent 是否仍在等待响应
 */

type LoadingMap = Map<string, { startTime: number; message?: string }>;

const loadingMap: LoadingMap = new Map();
...
/** 设置某个 agent 开始加载 */
export function setAgentLoading(agentId: string, message?: string) {
  loadingMap.set(agentId, { startTime: Date.now(), message });
  emitChange();
}

/** 设置某个 agent 加载完成 */
export function clearAgentLoading(agentId: string) {
...
/** 检查某个 agent 是否正在加载 */
export function isAgentLoading(agentId: string): boolean {
  return loadingMap.has(agentId);
}

/** 获取所有正在加载的 agent */
export function getLoadingAgents(): string[] {
  return Array.from(loadingMap.keys());
}

/** 获取加载中 agent 的数量（不包括当前显示的） */
export function getBackgroundLoadingCount(excludeAgentId?: string): number {
  let count = 0;
  loadingMap.forEach((_, id) => {
    if (id !== excludeAgentId) count++;
  });
  return count;
}

/** 获取加载详情 */
export function getLoadingInfo(agentId: string) {
  return loadingMap.get(agentId);
}

// 用于 useSyncExternalStore 的 snapshot
function getSnapshot(): LoadingMap {
  return loadingMap;
}
 ...
 */
export function useBackgroundLoading(currentAgentId?: string) {
  // 使用 useSyncExternalStore 订阅全局状态变化
  const map = useSyncExternalStore(subscribe, getSnapshot, getSnapshot);

  const allLoadingAgents = Array.from(map.keys());
  const backgroundLoadingAgents = allLoadingAgents.filter(
 ...
 */
export function useAgentLoading(agentId: string) {
  const map = useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
