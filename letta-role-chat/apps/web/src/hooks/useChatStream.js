...
/** 判断文本是否应转为附件 */
export function shouldConvertToAttachment(text) {
    if (!text)
        return false;
...
export function useChatStream({ role, chatId, onMessageSent, defaultAutoSpeak = false, onAutoSpeakChange, }) {
    ...
    const [textAttachments, setTextAttachments] = useState([]);
    ...
    const currentRoleIdRef = useRef(role.id);
    ...
    const handleAutoSpeakChange = useCallback((value) => {
        setAutoSpeak(value);
    ...
    useEffect(() => {
        setCollapsedMap((prev) => {
            ...
            Object.keys(next).forEach((k) => {
                if (!inputCodeCards.some((c) => c.id === k))
                    delete next[k];
            });
            return next;
        });
    }, [inputCodeCards]);
    const toggleCodeCard = useCallback((id) => {
    ...
    /** 处理粘贴的图片（剪贴板/截图） */
    const handlePastedImages = useCallback(async (files) => {
        // 过滤和验证文件
    ...
    const handlePastedText = useCallback((text) => {
    ...
    /** 删除文本附件 */
    const removeTextAttachment = useCallback((id) => {
    ...
    const upsertAssistantMessage = useCallback((id, content) => {
        setMessages((prev) => {
    ...
    const send = useCallback(async () => {
        ...
        if ((!input.trim() && uploadedImages.length === 0 && !hasTextAttachments) || isLoading)
            return;
