...
export function useTextSelectionTTS(containerRef, roleConfig, options = {}) {
    const { maxLen = 500, clearNativeSelectionOnClear = false, enableSelectionChange = true, } = options;
    const [sel, setSel] = useState({ text: "", rect: null });
    const [loading, setLoading] = useState(false);
    // selectionchange RAF 节流
    const rafIdRef = useRef(null);
    // 去重：避免同样文本频繁 setState
    const lastTextRef = useRef("");
    const normalize = useCallback((s) => s.replace(/\s+/g, " ").trim(), []);
    ...
    const stableRoleConfig = useMemo(() => roleConfig, [roleConfig.voice, roleConfig.speed, roleConfig.pitch, roleConfig.style]);
    ...
    const updateSelection = useCallback(() => {
        ...
        const commonEl = common.nodeType === 1 ? common : (common.parentElement ?? null);
        if (!commonEl || !el.contains(commonEl))
            return clear();
