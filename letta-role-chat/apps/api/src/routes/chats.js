...
const client_1 = require("../letta/client");
...
router.post("/:agentId", async (req, res) => {
    ...
    try {
        ...
        let lettaConversationId = null;
        // 调用 Letta API 创建新的 conversation
        // 这样可以确保每个对话有独立的上下文
        if (lettaAgentId) {
            try {
                const conversation = await client_1.lettaClient.conversations.create({
        ...
        const chatId = `chat_${(0, crypto_1.randomUUID)()}`;
        const now = Date.now();
        const chatTitle = title || "新对话";
        await db_1.default.query(`INSERT INTO chats (id, agent_id, letta_conversation_id, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`, [chatId, agentId, lettaConversationId, chatTitle, now, now]);
        const chat = {
            id: chatId,
            agent_id: agentId,
...
router.put("/:chatId", async (req, res) => {
    ...
    try {
        ...
        const [chats] = await db_1.default.query(`SELECT letta_conversation_id FROM chats WHERE id = ?`, [chatId]);
        ...
        if (lettaConversationId) {
            try {
                await client_1.lettaClient.conversations.update(lettaConversationId, {
                    summary: title
                });
        ...
        // 更新本地数据库
        const [result] = await db_1.default.query(`UPDATE chats SET title = ?, updated_at = ? WHERE id = ?`, [title, Date.now(), chatId]);
...
router.delete("/:chatId", async (req, res) => {
    ...
    try {
        ...
        if (lettaConversationId) {
            try {
                await client_1.lettaClient.conversations.cancel(lettaConversationId);
        ...
        // 删除聊天中的所有消息
        await db_1.default.query(`DELETE FROM messages WHERE chat_id = ?`, [chatId]);
        // 删除聊天
        const [result] = await db_1.default.query(`DELETE FROM chats WHERE id = ?`, [chatId]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: "Chat not found" });
        }
...
router.get("/:chatId/messages", async (req, res) => {
    const { chatId } = req.params;
    try {
        const [rows] = await db_1.default.query(`SELECT id, role, content, timestamp, images FROM messages WHERE chat_id = ? ORDER BY timestamp ASC`, [chatId]);
