...
router.post("/", async (req, res) => {
    try {
        ...
        const role = await agent_service_1.agentService.createRole(name, persona, human, voice, speed, pitch, style, avatarBase64);
...
router.put('/:roleId', async (req, res) => {
    ...
    const { name, persona, human, voice, speed, pitch, style, avatarBase64 } = req.body;
    try {
        const updated = await agent_service_1.agentService.updateRole(roleId, { name, persona, human, voice, speed, pitch, style, avatarBase64 });
        res.json(updated);
    }
    catch (error) {
