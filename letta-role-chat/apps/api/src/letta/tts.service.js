...
const TEMP_AUDIO_DIR = process.env.TEMP_AUDIO_DIR || path_1.default.resolve(__dirname, "..", "..", "temp_audio");
async function ensureTempDir() {
    await fs_1.default.promises.mkdir(TEMP_AUDIO_DIR, { recursive: true });
}
// 音频缓存管理（内存索引 + 定期清理）
const audioCache = new Map(); // fileName -> createdAt(ms)
...
const TEMP_AUDIO_CLEAN_INTERVAL_MS = Math.max(60000, Number(process.env.TEMP_AUDIO_CLEAN_INTERVAL_MS) || 5 * 60 * 1000);
// ✅ 新增：清理锁，防止并发清理
...
async function cleanupTempAudio() {
    ...
    try {
        await ensureTempDir();
        const files = await fs_1.default.promises.readdir(TEMP_AUDIO_DIR);
        const now = Date.now();
        // ✅ 步骤1：删除超过保留时长的文件（以内存索引时间优先，回退到文件 mtime）
        for (const f of files) {
            try {
                const p = path_1.default.join(TEMP_AUDIO_DIR, f);
                const stat = await fs_1.default.promises.stat(p).catch(() => null);
                const createdAt = audioCache.get(f) ?? stat?.mtimeMs ?? 0;
                if (now - createdAt > TEMP_AUDIO_RETENTION_MS) {
                    await fs_1.default.promises.unlink(p).catch(() => null);
                    audioCache.delete(f);
        ...
        const remaining = await fs_1.default.promises.readdir(TEMP_AUDIO_DIR);
        if (remaining.length > TEMP_AUDIO_MAX_FILES) {
            ...
            const stats = await Promise.all(remaining.map(async (f) => {
                const p = path_1.default.join(TEMP_AUDIO_DIR, f);
                const st = await fs_1.default.promises.stat(p).catch(() => null);
            ...
            for (const r of toRemove) {
                try {
                    const p = path_1.default.join(TEMP_AUDIO_DIR, r.file);
                    await fs_1.default.promises.unlink(p).catch(() => null);
        ...
        // ✅ 步骤3：清理内存索引中不存在的文件引用
        const actualFiles = new Set(await fs_1.default.promises.readdir(TEMP_AUDIO_DIR));
...
const WORKER_TTS_URL = process.env.WORKER_TTS_URL ||
...
async function resolveAgentTTSConfig(params) {
    ...
    if (agentId) {
        try {
            const agent = await agent_service_1.agentService.getRole(agentId);
            if (agent) {
                agentCfg = {
...
async function textToSpeechFile(params) {
    const { text, agentId, format = process.env.TTS_FORMAT || "mp3", timeoutMs = 30000, } = params;
    if (!text?.trim())
    ...
    let fileName = `${(0, uuid_1.v4)()}.${format}`;
    let filePath = path_1.default.join(TEMP_AUDIO_DIR, fileName);
    ...
    try {
        ...
        if (extByCT && !fileName.endsWith(`.${extByCT}`)) {
            fileName = `${(0, uuid_1.v4)()}.${extByCT}`;
            filePath = path_1.default.join(TEMP_AUDIO_DIR, fileName);
        }
        await ensureTempDir();
        try {
...
function contentTypeToExt(contentType) {
    ...
    if (ct.includes("audio/mpeg") || ct.includes("audio/mp3"))
        return "mp3";
    if (ct.includes("audio/opus"))
        return "opus";
    if (ct.includes("audio/aac"))
        return "aac";
